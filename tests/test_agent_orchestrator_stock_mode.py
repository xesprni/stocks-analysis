    ...
    def test_stock_mode_minimum_tool_chain(self):
        ...
        async def fake_financial_reports(**kwargs):
            ...
            return FinancialReportsResult(
                ...
                retrieved_at="2026-02-13T00:00:00+00:00",
                warnings=["empty_financial_reports"],
            )
        ...
        self.assertIn("get_fundamentals_info", tool_keys)
        self.assertIn("search_news", tool_keys)
        self.assertIn("search_web", tool_keys)
        self.assertIn("compute_indicators", tool_keys)
        self.assertIn("get_financial_reports", tool_keys)
        self.assertNotIn("get_filings", tool_keys)
        self.assertIn("## 结论摘要（3–6条）", result.final_report.markdown)

    def test_missing_evidence_markers_are_normalized_before_guardrails(self):
        orchestrator = AgentOrchestrator(
            config=AppConfig(),
            registry=ProviderRegistry(),
            news_service=_DummyNewsService(),
        ...
        self.assertNotIn("conclusion_without_evidence", codes)

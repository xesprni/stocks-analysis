...
class AgentOrchestratorSkillsTest(unittest.TestCase):
    def test_stock_skill_can_override_request_mode(self):
        ...
        async def fake_price(**kwargs):
            captured["price_interval"] = kwargs.get("interval")
        ...
        def fake_compute(price_df, indicators, symbol, indicator_profile):
            del price_df, indicators, symbol, indicator_profile
            return IndicatorsResult(
                symbol="AAPL",
                strategy={"score": 60, "stance": "neutral"},
                as_of="2026-02-21T00:00:00",
                source="computed",
                retrieved_at="2026-02-21T00:00:00+00:00",
                warnings=[],
        ...
        self.assertEqual(captured.get("runtime_mode"), "stock")
        self.assertEqual(result.analysis_input.get("mode"), "stock")
        self.assertIn(
            "get_price_history", result.analysis_input.get("tool_results", {})
        )

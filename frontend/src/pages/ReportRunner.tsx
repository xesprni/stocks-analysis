...
export function ReportRunnerPage({ onRunReport, running }: Props) {
  const [reportMode, setReportMode] = useState<"market" | "stock" | "watchlist">("market");
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-sky-300/50 bg-gradient-to-br from-sky-500/15 via-cyan-400/10 to-emerald-400/15 p-6">
        ...
        <div className="relative">
          ...
          <p className="mt-2 text-sm text-muted-foreground">集中执行市场、个股与持仓（watchlist）模式报告任务，结果在 Reports 页面查看。</p>
        </div>
      </section>
      ...
      <Card className="border-sky-200/60 bg-gradient-to-br from-white to-sky-50/40 dark:from-slate-900 dark:to-sky-950/20">
        <CardHeader>
          ...
          <CardDescription>支持市场模式（可指定 CN/HK/US）、个股模式与持仓报告（基于 Watchlist 已启用标的）。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="report_mode">报告模式</Label>
              <Select value={reportMode} onValueChange={(value: "market" | "stock" | "watchlist") => setReportMode(value)}>
            ...
            {reportMode === "watchlist" ? (
              <div className="space-y-2 md:col-span-2">
                <Label>Watchlist 范围</Label>
                <div className="rounded-md border bg-card px-3 py-2 text-sm text-muted-foreground">
                  将使用 Watchlist 中已启用的持仓标的生成组合报告。
                </div>
              </div>
            ) : null}
          </div>
            ...
            <Button
              ...
              onClick={() => {
                ...
                onRunReport({
                  ...
                  market: reportMode === "watchlist" ? undefined : reportMarket,
                  question: reportQuestion.trim() || undefined,
                  peer_list: reportMode === "stock" && peerList.length ? peerList : undefined,
                });
              }}
